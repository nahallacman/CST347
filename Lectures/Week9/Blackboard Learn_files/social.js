...
social.Profile =
{
  ...
  // id to the profile accessor HTML element (eg. id for anchor tag to My Profile icon)
  MY_PROFILE_ACCESSOR_ID: "profileAccessLink",
  ...
  // location of the profile accessor - global navigation menu
  LINK_ORG_MENU:          "globalNav",

  // location of the profile accessor - in My Blackboard
  LINK_ORG_MYBB:          "myBb",
...
social.Profile.toggleView = function( displayTarget, profilePageUri )
{
  ...
  top.ProfileProviderService.isValidProfileUri( profilePageUri, { callback :
      function( isValid )
      {
        if( isValid )
        {
          var myProfileAccessorElement = $( social.Profile.MY_PROFILE_ACCESSOR_ID );
          var accessorLocation = myProfileAccessorElement.readAttribute( "data-profile-link-location" );
          ...
          else if( displayTarget === social.Profile.DISPLAY_TARGET_MYBB )
          {
            ...
            var myBbProfileUri = social.Profile.MY_PROFILE_TOOL_URI + escape( profilePageUri );
            
            if( accessorLocation === social.Profile.LINK_ORG_MENU )
            {
              $("contentFrame").src = myBbProfileUri;
            }
            else if( accessorLocation === social.Profile.LINK_ORG_MYBB )
            {
              window.location = myBbProfileUri;
            }
          }
        }
        else
